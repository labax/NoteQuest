...
const persistenceFaultPoints: readonly PersistenceFaultPoint[] = [
  'transaction.before-transaction',
  'transaction.after-required-writes',
  'transaction.before-completion',
  'transaction.after-completion-before-receipt',
...
const injectedFaultName = ['Injected', 'Persistence', 'Fault'].join('');

function recognisedInjectedFault(
  cause: unknown,
): cause is Error & { point: PersistenceFaultPoint } {
  return (
    cause instanceof Error &&
    cause.name === injectedFaultName &&
    'point' in cause &&
    typeof cause.point === 'string' &&
    persistenceFaultPoints.includes(cause.point as PersistenceFaultPoint)
  );
}
...
export function injectedFaultMessage(cause: unknown): string | null {
  return recognisedInjectedFault(cause) ? cause.message : null;
}

export function injectedFaultPoint(cause: unknown): PersistenceFaultPoint | null {
  return recognisedInjectedFault(cause) ? cause.point : null;
}
